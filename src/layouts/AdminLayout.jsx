...
import {
  LayoutDashboard, Award, FilePlus, Users, BookOpen,
  Palette, ShieldCheck, BarChart3, Settings, Menu, X, LogOut
} from 'lucide-react';
...
const sidebarLinks = [
  ...
  { to: '/admin/certificates/issue', icon: FilePlus, label: 'Issue Certificate' },
  { to: '/admin/students', icon: Users, label: 'Students' },
  { to: '/admin/certifications', icon: BookOpen, label: 'Certifications' },
  { to: '/admin/templates', icon: Palette, label: 'Templates' },
...
export default function AdminLayout() {
  ...
  if (!user || (user.role !== 'ADMIN' && user.role !== 'ISSUER')) {
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-thin">
          {sidebarLinks.map(link => {
            if (link.icon === Users && user.role !== 'ADMIN') return null;
