...
export default function StudentDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Welcome, {user?.name?.split(' ')[0]}</h1>
        <p className="text-white/40 mt-1 font-normal">Your certification dashboard</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Certificates Earned', value: myCerts.length, icon: Award, gradient: 'from-primary-500/15 to-secondary-500/15', iconColor: 'text-primary-400' },
          { label: 'Active Certificates', value: activeCerts.length, icon: BookOpen, gradient: 'from-emerald-500/15 to-emerald-500/5', iconColor: 'text-emerald-400' },
          { label: 'Average Score', value: avgScore + '%', icon: TrendingUp, gradient: 'from-purple-500/15 to-purple-500/5', iconColor: 'text-purple-400' },
        ].map((stat, i) => (
      ...
      {/* Certificates */}
      <div>
        <h2 className="text-lg font-semibold text-white mb-4 tracking-tight">Your Certificates</h2>
        {myCerts.length === 0 ? (
              ...
              <motion.div key={cert.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Card hover className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="text-sm font-bold text-primary-400">{cert.score}%</span>
                  </div>
                  ...
                  <p className="text-xs text-white/25 font-mono mb-3">{cert.certificateId}</p>
