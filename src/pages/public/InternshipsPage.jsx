    ...
    <div className="py-20 bg-black min-h-screen" style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-14">
          ...
          <h1 className="text-4xl font-bold text-white tracking-tight">Internship Opportunities</h1>
          ...
          {internships.map((internship, i) => (
            ...
            >
              <Card hover className="p-6 h-full flex flex-col">
                ...
                <h3 className="text-lg font-bold text-white mb-2 tracking-tight">{internship.title}</h3>
                <div className="flex items-center gap-2 mb-3">
                  <Building2 className="w-4 h-4 text-white/30" />
                  <span className="text-sm text-white/40">{internship.company}</span>
                </div>
                <p className="text-sm text-white/40 leading-relaxed mb-4 flex-1 font-normal">{internship.description}</p>
                <div className="flex flex-wrap items-center gap-4 text-sm text-white/30 mb-4">
                ...
                <div className="flex flex-wrap gap-1.5 mb-5">
                  {internship.requirements.map(req => (
                    <span key={req} className="px-2 py-0.5 bg-white/[0.06] text-white/50 rounded-md text-xs font-medium border border-white/[0.06]">{req}</span>
                  ))}
                </div>
