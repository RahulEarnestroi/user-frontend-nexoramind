    ...
    <div className="py-20 bg-white dark:bg-black min-h-screen">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          ...
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white tracking-tight">About NexoraMind Tech</h1>
          <p className="mt-4 text-lg text-slate-500 dark:text-white/40 max-w-2xl mx-auto leading-relaxed font-normal">
          ...
          {cards.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5 }}>
              <Card hover className="p-6 h-full">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white mb-2 tracking-tight">{item.title}</h3>
                <p className="text-sm text-slate-500 dark:text-white/40 leading-relaxed font-normal">{item.desc}</p>
              </Card>
            </motion.div>
          ))}
        </div>
