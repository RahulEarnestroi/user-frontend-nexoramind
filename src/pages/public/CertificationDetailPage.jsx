  ...
  if (!cert) {
    return (
      <div className="py-20 bg-black min-h-screen text-center" style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
        <h1 className="text-2xl font-bold text-white">Certification not found</h1>
        <Link to="/certifications" className="mt-4 inline-block text-primary-400 hover:underline">Back to certifications</Link>
      </div>
    );
  }
  ...
  return (
    <div className="py-16 bg-black min-h-screen" style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link to="/certifications" className="inline-flex items-center gap-1 text-sm text-white/40 hover:text-white mb-6 transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          ...
          <h1 className="text-4xl font-bold text-white tracking-tight">{cert.name}</h1>
          <p className="mt-4 text-lg text-white/50 leading-relaxed max-w-3xl font-normal">{cert.description}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
          <h2 className="text-2xl font-bold text-white mb-6 tracking-tight">Skills Covered</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {cert.skills.map(skill => (
              <div key={skill} className="flex items-center gap-2 px-4 py-3 bg-white/[0.03] rounded-lg border border-white/[0.06]">
        ...
        {/* Process */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
          <h2 className="text-2xl font-bold text-white mb-6 tracking-tight">Certification Process</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              {steps.map((step, i) => (
                <div key={i} className="relative text-center">
                  ...
                  <p className="text-xs text-white/30 mt-1 font-normal">{step.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Certificate Preview */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
          <h2 className="text-2xl font-bold text-white mb-6 tracking-tight">Certificate Preview</h2>
          <Card className="p-8">
            <div className="border-2 border-dashed border-primary-500/20 rounded-xl p-8 text-center bg-white/[0.02]">
              ...
              <p className="text-3xl font-bold text-white my-2 tracking-tight">Student Name</p>
              <p className="text-xs text-white/30">has successfully completed</p>
              <p className="text-lg font-semibold text-primary-400 mt-1">{cert.name}</p>
              <div className="mt-6 flex items-center justify-center gap-8 text-xs text-white/30">
                <div><span className="block font-medium text-white/50">Score</span>92%</div>
                <div><span className="block font-medium text-white/50">Issue Date</span>August 11, 2026</div>
                <div><span className="block font-medium text-white/50">Certificate ID</span>RC-2026-000184</div>
              </div>
            </div>
          </Card>
        </motion.div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, i) => (
              <Card key={i} className="p-6">
                ...
                <p className="text-sm text-white/40 font-normal">{faq.a}</p>
              </Card>
            ))}
          </div>
        </motion.div>
