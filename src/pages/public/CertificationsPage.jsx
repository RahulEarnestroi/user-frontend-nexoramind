    ...
    <div className="py-20 bg-gradient-subtle min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-14">
          ...
          <h1 className="text-4xl font-bold text-slate-900">Certification Programs</h1>
              ...
              <Card hover className="p-6 h-full flex flex-col">
                ...
                <div className="flex flex-wrap gap-1.5 mb-5">
                  {cert.skills.slice(0, 4).map(skill => (
                    <span key={skill} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md text-xs font-medium">{skill}</span>
                  ))}
                  {cert.skills.length > 4 && (
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-xs">+{cert.skills.length - 4}</span>
                  )}
                </div>
